...
try:
    ...
    fh = open(r'C:\Users\thartley\Desktop\LocationHistory.json')
buf = fh.read()
raw = json.loads(buf)
fh.close()
...
plt.plot(ts.index, range(len(ts))[::-1])
...
shapefilename = helpers.user_prefix() + r'\data\Municipal Boundary\Municipal Boundary'
...
out = m.readshapefile(shapefilename, name='stlouis', drawbounds=False, color='none', zorder=2)

# <codecell>

# set up a map dataframe
df_map = pd.DataFrame({
    'poly': [Polygon(hood_points) for hood_points in m.stlouis],
...
hx = m.hexbin(
    ...
    gridsize=(numhexbins, int(numhexbins*h/w*0.9)), #critical to get regular hexagon, must stretch to map dimensions
...
fig.suptitle("My location density in Seattle", fontdict={'size':24, 'fontweight':'bold'}, y=0.98)
ax.set_title("Using location data collected from my Android phone via Google Takeout", fontsize=14, y=0.98)
...
plt.savefig('hexbin.png', dpi=300, frameon=False, bbox_inches='tight', pad_inches=0.5, facecolor='#DEDEDE')
